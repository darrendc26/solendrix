...
pub struct Invest<'info> {
    ...
    /// CHECK: This is only checked for ownership, not deserialized
    #[account(mut)]
    ...
    /// CHECK: This is a PDA for receiving investments
    pub sol_vault: AccountInfo<'info>,

    #[account(address = LIGHT_PROTOCOL_ID)]
    /// CHECK: This is a program account
...
pub fn invest_handler<'a, 'b, 'c, 'info>(
    ...
    let ico_tokens = amount.checked_div(ico.price_lamports).ok_or(ErrorCode::OverflowError)?;
    ...
    require!(ico_tokens <= ico.total_tokens, ErrorCode::NotEnoughTokens); 

    // Validate output merkle tree index is within bounds
    require!(output_merkle_tree_index < 32, ErrorCode::InvalidMerkleTreeIndex);
    
    let investor_balance = ctx.accounts.investor.to_account_info().lamports();
    require!(investor_balance >= amount, ErrorCode::InsufficientFunds);
    
    // === 1. PRIVATE SOL TRANSFER ===
    // Transfer SOL from investor to ICO vault
    ...
    investment.account.amount = ico_tokens;
