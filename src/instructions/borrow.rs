use pinocchio::{account_info::AccountInfo, program_error::ProgramError, ProgramResult,
    pubkey::{find_program_address}, instruction::{Seed, Signer}};
...
impl<'a> TryFrom<&'a [AccountInfo]> for BorrowAccounts<'a> {
    ...
    fn try_from(accounts: &'a [AccountInfo]) -> Result<Self, Self::Error> {
        let [user, admin, user_pda, market, user_token_account_b, mint_b, vault_b, system_program, token_program] = accounts else {
...
impl<'a> Borrow<'a> {
    ...
    pub fn process(&self) -> ProgramResult {
        ...
        market.set_total_withdrawals(market.total_withdrawals + amount);
