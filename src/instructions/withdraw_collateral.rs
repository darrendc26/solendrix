...
impl<'a> TryFrom<(&'a [u8], &'a [AccountInfo])> for WithdrawCollateral<'a> {
    ...
    fn try_from((data, accounts): (&'a [u8], &'a [AccountInfo])) -> Result<Self, Self::Error> {
        ...
        let (user_pda, _bump) =
            find_program_address(&[b"user", accounts.user.key().as_ref()], &crate::ID);
        if user_pda != *accounts.user_pda.key() {
            return Err(ProgramError::InvalidSeeds);
        }

        let (market_pda, _market_bump) =
            find_program_address(&[b"market", accounts.admin.key().as_ref()], &crate::ID);
    ...
    pub fn process(&self) -> ProgramResult {
        {
            ...
            let (_market_pda, market_bump) =
                find_program_address(&[b"market", self.accounts.admin.key().as_ref()], &crate::ID);

            let seed_binding = [market_bump];
            let market_seeds = [
                Seed::from(b"market"),
